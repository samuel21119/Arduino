...
int detection()    // 超音波偵測函數 三個角度讀取距離數值
{
  ...
  int L_Distance = 0;
  if (temp == 1) {
    F_DistanceLEFT = Ask_Distance(120);
    if (F_DistanceLEFT < 10) {
      goto depend;
    }
    F_DistanceMID = Ask_Distance(100);
    if (F_DistanceMID < 10) {
      goto depend;
    }
    F_DistanceRIGHT = Ask_Distance(80);
...
depend:
  ...
  if (F_DistanceMID < 10) // 超音波感測器偵測後得到的距離低於 10公分
  {
    stopRL(1);
    back(2);
  }
  if (F_DistanceLEFT < 25 || F_DistanceMID < 25 || F_DistanceRIGHT < 25) // 超音波感測器偵測前方的距離小於25公分
  {
    stopRL(1);

    R_Distance = Ask_Distance(10);    // 讀取右方距離, 請依照您組裝後的實際狀況調整舵機角度
    L_Distance = Ask_Distance(190);  // 讀取左方距離, 請依照您組裝後的實際狀況調整舵機角度
    temp = 2;
